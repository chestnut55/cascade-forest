...
if __name__ == "__main__":
    ...
    config = load_json("/home/qiang/repo/python2/cascade_clf/examples/demo_ca.json")
    gc = GCForest(config)
    ...
    for dataset_idx, name in enumerate(datasets):
        ...
        for train, test in cv.split(X, Y):
            ...
            X_train_enc, _features = gc.fit_transform(X_train, y_train,threshold=None)

            print(_features.size)
            save_features(name + "-cv-" + str(i), _features.to_dict())
