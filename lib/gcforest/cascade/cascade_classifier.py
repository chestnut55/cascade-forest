...
class CascadeClassifier(object):
    ...
    def _init_estimators(self, li, ei):
        ...
        # n_folds
        LOGGER.info("=====est_name============{}".format(est_name))
        ...
        for level, feat in features.items():
            if int(level) <= opt_layer_id:
                if int(level) > 0:
                    ...
                    feat = feat.loc[[int(index) >= n_meta_features for index in feat.index.tolist()]]
                    print("size="+str(feat.size))
                    values = feat.values
                    index = [int(x) - n_meta_features for x in feat.index.tolist()]
                    index = [str(idx) for idx in index]
                    new_feat = pd.Series(data=values, index=index)
                    feats = feats.add(new_feat, fill_value=0)
                else:
